...
=end
  ...
  def fn_lodsw
    @reg_ax = eval(@reg_si) 
    @reg_si = fn_inc_si(@reg_si)
end
  
  def fn_inc_si(reg)
    if /\.pfa/.match(word_sub)
       @prog_count += 1
  ...
  def pick_word(reg)
    @word_name = (/[^\.]+/.match(reg)).to_s
    @word_sub  = (/\.[^\.]+/.match(reg)).to_s
  end
  def pick_next_word(reg)	  
    @next_word_name = (/[^\.]+/.match(reg)).to_s
    @next_word_sub  = (/\.[^\.]+/.match(reg)).to_s
  end
  def fn_push(reg)
    @stack[@reg_sp] = reg
    @reg_sp += 1
...
<<"EOL"
    ...
    eval(@reg_bp)+= @reg_bx
	@reg_ax = eval(@reg_bp)
	@reg_ax -= eval(inc_bx(@reg_bp))
	@reg_ax = @reg_ax ^ @reg_bx
	if 
